...
function siguienteIteracion() {
	iteraccionActual =
		iteraccionActual + 1 * (iteraccionActual < resultadoActual.length - 1);
...
function resolver() {
	...
	try {
		...
		actualizarFuncion(
			MathExpression.fromLatex(mathField.latex()).toString().toLowerCase()
		);
	}
	triggerBotones(true);
}
...
if (navigator.userAgent.match(/Mobile/)) {
	document.getElementById("cuelloBotella").innerHTML =
		'<input type="text" id="funcion" value="2*sin(x)-(x^2)/10" onchange="actualizarFuncion(this.value)">';
}
